...
```text
请输入结束内网 IP 末位，默认 254（范围 2-254，直接回车使用默认）：
```

例如输入 `80`，就只生成 `10.0.0.2 - 10.0.0.80` 的转发规则；直接回车则生成到 `10.0.0.254`。
...
```bash
sudo IP_PREFIX=10.0.0 IP_START=2 IP_END=254 ENABLE_UDP=1 ./install_gost_forward.sh
```

常用变量：

- `IP_PREFIX`：目标 IP 前缀，默认 `10.0.0`
- `IP_START` / `IP_END`：目标 IP 末位范围，默认 `2` 到 `254`
